...
func NewUnboundedCanvasObjectChan() *UnboundedCanvasObjectChan {
	ch := &UnboundedCanvasObjectChan{
		...
		in:    make(chan gui.CanvasObject, 16),
		out:   make(chan gui.CanvasObject, 16),
...
func (ch *UnboundedCanvasObjectChan) In() chan<- gui.CanvasObject { return ch.in }

// Out returns the receive channel of the given channel, which can be used
// to receive values from the channel.
func (ch *UnboundedCanvasObjectChan) Out() <-chan gui.CanvasObject { return ch.out }
...
func (ch *UnboundedCanvasObjectChan) processing() {
	...
	ch.q = make([]gui.CanvasObject, 0, 1<<10)
	for {
		select {
		case e, ok := <-ch.in:
			if !ok {
